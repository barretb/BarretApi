...
public sealed class RssRandomPostService_TagExclusion_Tests
{
    ...
    [Fact]
    public async Task IncludesEntriesWithoutTags_GivenTagFilterRemoved()
    {
    ...
    [Fact]
    public async Task SelectsEntry_GivenAllEntriesHaveNoTags()
    {
        ...
        result.SelectedEntry.EntryIdentity.ShouldBe("no-tags-1");
    }

    [Fact]
    public async Task SelectsAnyEntry_GivenMixOfTaggedAndUntaggedEntries()
    {
        ...
        new[] { "tagged", "untagged" }.ShouldContain(result.SelectedEntry.EntryIdentity);
    }
...
public sealed class RssRandomPostService_HeaderPrepend_Tests
{
    private readonly IBlogFeedReader _feedReader = Substitute.For<IBlogFeedReader>();
    private readonly ISocialPlatformClient _mockClient;
    private readonly RssRandomPostService _sut;
    ...
    public RssRandomPostService_HeaderPrepend_Tests()
    {
        _mockClient = CreateMockClient("testplatform");
    ...
    [Fact]
    public async Task PrependsHeader_GivenHeaderProvided()
    {
        _feedReader.ReadEntriesAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(SingleEntry);
        ...
        await _mockClient.Received(1).PostAsync(
            Arg.Is<string>(text =>
                ...
                text.IndexOf("Check this out!", StringComparison.Ordinal) &&
                text.IndexOf("Check this out!", StringComparison.Ordinal) <
                text.IndexOf("Test Post", StringComparison.Ordinal)),
            Arg.Any<IReadOnlyList<UploadedImage>>(),
            Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task OmitsHeader_GivenNullHeader()
    {
        _feedReader.ReadEntriesAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(SingleEntry);
    ...
    [Fact]
    public async Task OmitsHeader_GivenWhitespaceHeader()
    {
        _feedReader.ReadEntriesAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(SingleEntry);
    ...
    [Fact]
    public async Task SelectsTaglessEntry_GivenNoExcludeTags()
    {
        var taglessEntries = new List<BlogFeedEntry>
        {
            new()
            {
                EntryIdentity = "tagless",
                CanonicalUrl = "https://example.com/tagless",
                Title = "Tagless Post",
                PublishedAtUtc = DateTimeOffset.UtcNow.AddDays(-1),
                Tags = []
            }
        ...
        result.SelectedEntry.EntryIdentity.ShouldBe("tagless");
    }
