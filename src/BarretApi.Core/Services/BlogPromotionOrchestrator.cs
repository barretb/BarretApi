    ...
    ILogger<BlogPromotionOrchestrator> logger) : IBlogPromotionOrchestrator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var effectiveFeedUrl = string.IsNullOrWhiteSpace(feedUrl) ? _options.FeedUrl : feedUrl;

        List<BlogFeedEntry> feedEntries;
        try
        {
            feedEntries = (await _blogFeedReader.ReadEntriesAsync(effectiveFeedUrl, cancellationToken)).ToList();
        }
        catch (Exception ex)
        {
        ...
        var effectiveRecentDaysWindow = recentDaysWindow ?? _options.RecentDaysWindow;
        var cutoff = now.AddDays(-effectiveRecentDaysWindow);
        ...
        foreach (var entry in feedEntries.OrderByDescending(e => e.PublishedAtUtc))
        {
            ...
            var initialResults = await _socialPostService.PostAsync(
                new SocialPost
                {
                    Text = BuildInitialPostText(entry, header),
                    Hashtags = entry.Tags,
        ...
        if (_options.EnableReminderPosts)
        {
            ...
            foreach (var trackedRecord in trackedRecords)
            {
                ...
                var reminderResults = await _socialPostService.PostAsync(
                    new SocialPost
                    {
                        Text = BuildReminderPostText(trackedRecord, header),
    ...
    private static string BuildInitialPostText(BlogFeedEntry entry, string? header)
    {
        var baseText = $"{entry.Title}\n{entry.CanonicalUrl}";
        return string.IsNullOrWhiteSpace(header) ? baseText : $"{header}\n\n{baseText}";
    }

    private static string BuildReminderPostText(BlogPostPromotionRecord record, string? header)
    {
        var baseText = $"In case you missed it earlier...\n\n{record.Title}\n{record.CanonicalUrl}";
        return string.IsNullOrWhiteSpace(header) ? baseText : $"{header}\n\n{baseText}";
    }
